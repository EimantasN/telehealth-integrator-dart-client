...
class GraphApi {
  ...
  /// Performs an HTTP 'POST /api/Graph/Sleep' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [GetSleepDataQuery] getSleepDataQuery (required):
  Future<Response> graphSleepWithHttpInfo(GetSleepDataQuery getSleepDataQuery,) async {
    // ignore: prefer_const_declarations
    final path = r'/api/Graph/Sleep';
  ...
  ///
  /// * [GetSleepDataQuery] getSleepDataQuery (required):
  Future<SleepDataDto?> graphSleep(GetSleepDataQuery getSleepDataQuery,) async {
    final response = await graphSleepWithHttpInfo(getSleepDataQuery,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
